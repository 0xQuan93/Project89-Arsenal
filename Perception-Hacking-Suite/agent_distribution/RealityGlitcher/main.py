...
class UIHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def __init__(self, *args, **kwargs):
        # Check if build directory exists, if not use src as fallback
        ui_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reality-glitcher-ui")
        build_dir = os.path.join(ui_dir, "build")
        src_dir = os.path.join(ui_dir, "src")
        
        if os.path.exists(build_dir) and os.path.isdir(build_dir):
            directory = build_dir
            logger.info("Using production build directory for React UI")
        else:
            # Build directory doesn't exist, use src as fallback
            directory = src_dir
            logger.warning("UI build directory not found. Using source directory as fallback.")
            logger.warning("For a proper UI experience, please build the React app using 'npm run build'")
            
        super().__init__(*args, directory=directory, **kwargs)

    def do_GET(self):
        """Override GET method to handle SPA routing"""
