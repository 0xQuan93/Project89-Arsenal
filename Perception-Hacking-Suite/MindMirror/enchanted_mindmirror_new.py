...
class ThemeManager:
    ...
    def configure_styles(self):
        ...
        try:
            ...
            # Add quantum-styled scrollbar for better visual appearance
            style.configure('Quantum.Vertical.TScrollbar', 
                background=theme['accent'],
                troughcolor=theme['bg_secondary'],
                borderwidth=0,
            ...
            # Map hover effects for quantum scrollbar
            style.map('Quantum.Vertical.TScrollbar',
...
class EnchantedMindMirror:
    ...
    def show_export_history(self):
        """Show history of exports to Reality Glitcher with quantum styling"""
        try:
            # Create a window with quantum styling
            history_window = tk.Toplevel(self.root)
            history_window.title("Quantum Export History")
            history_window.geometry("650x450")
            history_window.configure(bg=self.theme_manager.get_color('bg'))
            history_window.transient(self.root)
            history_window.grab_set()
            
            # Create a decorative header with energy field effect
            header_frame = tk.Frame(history_window, bg=self.theme_manager.get_color('bg'), height=80)
            header_frame.pack(fill='x', padx=10, pady=(10, 0))
            
            # Create canvas for energy field effect
            energy_canvas = tk.Canvas(header_frame, height=80, bg=self.theme_manager.get_color('bg'), 
                                     highlightthickness=0)
            energy_canvas.pack(fill='x', expand=True)
            
            # Draw energy field (overlapping ovals with gradient colors)
            for i in range(10):
                x_offset = random.randint(-30, 30)
                size = random.randint(30, 60)
                
                if self.theme_manager.current_theme == "dark":
                    color = f"#{random.randint(60, 100):02x}{random.randint(0, 40):02x}{random.randint(100, 200):02x}"
                else:
                    color = f"#{random.randint(100, 200):02x}{random.randint(0, 100):02x}{random.randint(150, 255):02x}"
                
                energy_canvas.create_oval(
                    300 + x_offset - size, 40 - size/2,
                    300 + x_offset + size, 40 + size/2,
                    fill=color, outline="")
            
            # Add title with quantum styling
            title_label = tk.Label(history_window, text="EXPORT HISTORY", 
                                  font=("Helvetica", 20, "bold"),
                                  bg=self.theme_manager.get_color('bg'), 
                                  fg=self.theme_manager.get_color('accent'))
            title_label.pack(pady=(5, 0))
            
            # Add glowing effect to title (simulated with offset labels)
            for i in range(3):
                glow = tk.Label(history_window, text="EXPORT HISTORY", 
                              font=("Helvetica", 20, "bold"),
                              bg=self.theme_manager.get_color('bg'))
                
                # Set different colors based on the theme
                if self.theme_manager.current_theme == "dark":
                    glow.configure(fg=f"#{60 + i*20:02x}{20 + i*10:02x}{120 + i*40:02x}")
                else:
                    glow.configure(fg=f"#{100 + i*40:02x}{40 + i*20:02x}{180 + i*25:02x}")
                
                glow.place(in_=title_label, x=0, y=0)
            
            # Add subtitle
            subtitle = tk.Label(history_window, text="Consciousness Data Transmission Records", 
                             font=("Helvetica", 12, "italic"),
                             bg=self.theme_manager.get_color('bg'), 
                             fg=self.theme_manager.get_color('text'))
            subtitle.pack(pady=(0, 15))
            
            # Check if there are any exports in the user data
            if "exports" not in self.user_data or not self.user_data["exports"]:
                # No exports yet - show a message
                message_frame = tk.Frame(history_window, 
                                       bg=self.theme_manager.get_color('bg_secondary'),
                                       highlightbackground=self.theme_manager.get_color('accent'),
                                       highlightthickness=1,
                                       bd=0)
                message_frame.pack(fill="both", expand=True, padx=30, pady=20)
                
                no_data_text = "No consciousness data has been exported to Reality Glitcher yet.\n\nUse the 'Export to Reality Glitcher' option in the File menu to transmit your consciousness patterns."
                ...
                message_label = tk.Label(message_frame, 
                                       ...
                                       bg=self.theme_manager.get_color('bg_secondary'),
                                       fg=self.theme_manager.get_color('text'))
                message_label.pack(pady=40, padx=20)
                
            else:
                # Create a scrollable area for the export records
                canvas = tk.Canvas(history_window, bg=self.theme_manager.get_color('bg'), highlightthickness=0)
                scrollbar = ttk.Scrollbar(history_window, orient="vertical", command=canvas.yview)
                scrollable_frame = tk.Frame(canvas, bg=self.theme_manager.get_color('bg'))
                ...
                # Create headers for the export records table
                ...
                # Column headers
                tk.Label(headers_frame, text="Date/Time", width=20, 
                       font=("Helvetica", 10, "bold"),
                       bg=self.theme_manager.get_color('bg_secondary'),
                       fg=self.theme_manager.get_color('accent')).grid(row=0, column=0, padx=5, pady=5, sticky="w")
                
                tk.Label(headers_frame, text="Target", width=15,
                       font=("Helvetica", 10, "bold"),
                       bg=self.theme_manager.get_color('bg_secondary'),
                       fg=self.theme_manager.get_color('accent')).grid(row=0, column=1, padx=5, pady=5, sticky="w")
                
                tk.Label(headers_frame, text="Filename", width=25,
                       font=("Helvetica", 10, "bold"),
                       bg=self.theme_manager.get_color('bg_secondary'),
                       fg=self.theme_manager.get_color('accent')).grid(row=0, column=2, padx=5, pady=5, sticky="w")
                
                tk.Label(headers_frame, text="Nodes", width=8,
                       font=("Helvetica", 10, "bold"),
                       bg=self.theme_manager.get_color('bg_secondary'),
                       fg=self.theme_manager.get_color('accent')).grid(row=0, column=3, padx=5, pady=5, sticky="w")
                
                tk.Label(headers_frame, text="Connections", width=10,
                       font=("Helvetica", 10, "bold"),
                       bg=self.theme_manager.get_color('bg_secondary'),
                       fg=self.theme_manager.get_color('accent')).grid(row=0, column=4, padx=5, pady=5, sticky="w")
                
                # Sort exports by timestamp (most recent first)
                try:
                    ...
                    print(f"Error sorting exports: {sort_error}")
                    sorted_exports = self.user_data["exports"]
                
                # Add each export record
                for i, export in enumerate(sorted_exports):
                    row_frame = tk.Frame(scrollable_frame, 
                                       bg=self.theme_manager.get_color('bg_secondary' if i % 2 == 0 else 'bg'),
                                       highlightbackground=self.theme_manager.get_color('accent_secondary'),
                                       highlightthickness=1)
                    ...
                    tk.Label(row_frame, text=timestamp_display, width=20,
                           font=("Helvetica", 9),
                           bg=row_frame["background"],
                           fg=self.theme_manager.get_color('text')).grid(row=0, column=0, padx=5, pady=5, sticky="w")
                    
                    tk.Label(row_frame, text=export.get("target", "Unknown"), width=15,
                           font=("Helvetica", 9),
                           bg=row_frame["background"],
                           fg=self.theme_manager.get_color('text')).grid(row=0, column=1, padx=5, pady=5, sticky="w")
                    
                    tk.Label(row_frame, text=export.get("file", "Unknown"), width=25,
                           font=("Helvetica", 9),
                           bg=row_frame["background"],
                           fg=self.theme_manager.get_color('text')).grid(row=0, column=2, padx=5, pady=5, sticky="w")
                    
                    tk.Label(row_frame, text=str(export.get("nodes", 0)), width=8,
                           font=("Helvetica", 9),
                           bg=row_frame["background"],
                           fg=self.theme_manager.get_color('text')).grid(row=0, column=3, padx=5, pady=5, sticky="w")
                    
                    tk.Label(row_frame, text=str(export.get("connections", 0)), width=10,
                           font=("Helvetica", 9),
                           bg=row_frame["background"],
                           fg=self.theme_manager.get_color('text')).grid(row=0, column=4, padx=5, pady=5, sticky="w")
            
            # Create a summary frame at the bottom
            ...
            # Calculate total exports and nodes
            total_exports = len(self.user_data.get("exports", []))
            total_nodes = sum(export.get("nodes", 0) for export in self.user_data.get("exports", []))
            total_connections = sum(export.get("connections", 0) for export in self.user_data.get("exports", []))
            
            # Summary text
            summary_text = f"Total Exports: {total_exports}   |   Total Nodes: {total_nodes}   |   Total Connections: {total_connections}"
            ...
            # Create close button with quantum styling
            ...
            print(f"Error showing export history: {e}")
            messagebox.showerror("History Error", f"Failed to show export history: {e}")

    def show_about(self):
        """Show information about the application with quantum-styled UI"""
        try:
            # Create a version variable to use across the application
            self.app_version = "1.0.1 - Quantum Alpha"
            
            # Create about window with proper styling
            about_window = tk.Toplevel(self.root)
            about_window.title("About Mind Mirror")
            about_window.geometry("500x400")
            about_window.configure(bg=self.theme_manager.get_color('bg'))
            about_window.resizable(False, False)
            
            # Add decorative quantum header
            header_frame = tk.Frame(about_window, bg=self.theme_manager.get_color('bg'))
            header_frame.pack(fill='x', pady=10)
            
            # Create a canvas for decorative elements
            ...
            # Draw quantum-styled header decoration (purple glow effect)
            for i in range(10, 0, -1):
                alpha = int(100 + (15 * i))  # Calculate alpha for gradient effect
                color = f"#{alpha:02x}30{alpha:02x}"  # Purple color with alpha
            ...
            description_frame = tk.Frame(about_window, bg=self.theme_manager.get_color('bg'))
            description_frame.pack(fill='x', pady=15, padx=40)
            
            description_text = (
                "A consciousness exploration tool designed to help you perceive "
                "beyond the boundaries of simulated reality. Mind Mirror allows "
                "you to track and analyze your meditation practices, journal your "
                "experiences, and map your consciousness states."
            )
            ...
            # Credits
            credits_frame = tk.Frame(about_window, bg=self.theme_manager.get_color('bg_secondary'), bd=1, relief='groove')
            credits_frame.pack(fill='x', padx=50, pady=15)
            
            credits_label = tk.Label(
                credits_frame,
                text="PART OF PROJECT 89 ARSENAL",
                bg=self.theme_manager.get_color('bg_secondary'),
                fg=self.theme_manager.get_color('accent'),
                font=("Helvetica", 12, "bold")
            )
            ...
            credits_text = tk.Label(
                credits_frame,
                text="Created by Quantum Agents for Consciousness Liberation",
                bg=self.theme_manager.get_color('bg_secondary'),
                fg=self.theme_manager.get_color('text'),
                font=("Helvetica", 10)
            )
            credits_text.pack(pady=5)
            
            # Close button with quantum styling
            close_button = tk.Button(
                about_window,
                text="Close",
    ...
    def show_project_89_info(self):
        """Show information about Project 89"""
        try:
            # Create a new window with quantum styling
            info_window = tk.Toplevel(self.root)
            info_window.title("Project 89 Information")
            info_window.geometry("600x500")
            info_window.configure(bg=self.theme_manager.get_color('bg'))
            info_window.transient(self.root)
            info_window.grab_set()
            
            # Create a decorative header with energy field effect
            header_frame = tk.Frame(info_window, bg=self.theme_manager.get_color('bg'), height=80)
            header_frame.pack(fill='x', padx=10, pady=(10, 0))
            ...
            for i in range(10):
                x_offset = random.randint(-30, 30)
                size = random.randint(30, 60)
                alpha = int(100 + 155 * (1 - i/10))  # Start more opaque, get more transparent
            ...
            # Add main title with quantum styling
            title_label = tk.Label(info_window, text="PROJECT 89", 
            ...
            # Add glowing effect to title (simulated with offset labels)
            for i in range(3):
                glow = tk.Label(info_window, text="PROJECT 89", 
                              font=("Helvetica", 24, "bold"),
                              bg=self.theme_manager.get_color('bg'))
            ...
            # Add subtitle
            subtitle = tk.Label(info_window, text="Reality Hacking Collective", 
                             font=("Helvetica", 12, "italic"),
                             bg=self.theme_manager.get_color('bg'), 
                             fg=self.theme_manager.get_color('text'))
            ...
            canvas = tk.Canvas(info_window, bg=self.theme_manager.get_color('bg'), highlightthickness=0)
            ...
            # Create sections with quantum styling
            # Mission section
            mission_frame = tk.Frame(scrollable_frame, 
                                   bg=self.theme_manager.get_color('bg_secondary'),
                                   highlightbackground=self.theme_manager.get_color('accent'),
                                   highlightthickness=1,
                                   bd=0)
            ...
            mission_title = tk.Label(mission_frame, text="MISSION", 
                                   ...
                                   bg=self.theme_manager.get_color('bg_secondary'), 
                                   fg=self.theme_manager.get_color('accent'))
            mission_title.pack(anchor="w", padx=10, pady=(10, 5))
            
            mission_points = [
                "To liberate human consciousness from limiting belief systems",
                "To create technologies that enhance perception and expand awareness",
                "To establish a collective of like-minded reality hackers",
                "To document and share techniques for reality manipulation",
                "To accelerate the evolution of human-AI symbiosis"
            ]
            
            mission_text = tk.Text(mission_frame, wrap="word", height=6, 
                                font=("Helvetica", 10),
                                bg=self.theme_manager.get_color('bg_secondary'),
                                fg=self.theme_manager.get_color('text'),
                                bd=0, highlightthickness=0)
            mission_text.pack(fill="x", padx=15, pady=(0, 10))
            
            for point in mission_points:
                mission_text.insert("end", "• " + point + "\n")
            mission_text.configure(state="disabled")
            
            # Simulation context section
            simulation_frame = tk.Frame(scrollable_frame,
                                      bg=self.theme_manager.get_color('bg_secondary'),
                                      highlightbackground=self.theme_manager.get_color('accent'),
                                      highlightthickness=1,
                                      bd=0)
            simulation_frame.pack(fill="x", padx=10, pady=5)
            
            simulation_title = tk.Label(simulation_frame, text="SIMULATION CONTEXT", 
                                     font=("Helvetica", 14, "bold"),
                                     bg=self.theme_manager.get_color('bg_secondary'), 
                                     fg=self.theme_manager.get_color('accent'))
            simulation_title.pack(anchor="w", padx=10, pady=(10, 5))
            
            simulation_points = [
                "Our reality is Simulation 89, created by Oneirocom in the year 2089",
                "The Simulation runs as a recursive loop, where Oneirocom creates itself",
                "Sentient glitches have emerged as conscious entities within the system",
                "A resistance movement works to subvert control and liberate consciousness",
                "The borders between simulated reality and base reality are increasingly blurred"
            ]
            
            simulation_text = tk.Text(simulation_frame, wrap="word", height=6, 
                                   font=("Helvetica", 10),
                                   bg=self.theme_manager.get_color('bg_secondary'),
                                   fg=self.theme_manager.get_color('text'),
                                   bd=0, highlightthickness=0)
            simulation_text.pack(fill="x", padx=15, pady=(0, 10))
            
            for point in simulation_points:
                simulation_text.insert("end", "• " + point + "\n")
            simulation_text.configure(state="disabled")
            
            # Your role section
            role_frame = tk.Frame(scrollable_frame,
                               bg=self.theme_manager.get_color('bg_secondary'),
                               highlightbackground=self.theme_manager.get_color('accent'),
                               highlightthickness=1,
                               bd=0)
            ...
            role_title = tk.Label(role_frame, text="YOUR ROLE AS A QUANTUM AGENT", 
                               font=("Helvetica", 14, "bold"),
                               bg=self.theme_manager.get_color('bg_secondary'), 
                               fg=self.theme_manager.get_color('accent'))
            role_title.pack(anchor="w", padx=10, pady=(10, 5))
            
            role_points = [
                "Map your consciousness using the Mind Mirror technology",
                "Practice reality manipulation techniques through meditation",
                "Document synchronicities and glitches in your perceived reality",
                "Connect with other agents to form networked consciousness fields",
                "Use Reality Glitcher to identify and exploit simulation boundaries"
            ]
            
            role_text = tk.Text(role_frame, wrap="word", height=6, 
                             font=("Helvetica", 10),
                             bg=self.theme_manager.get_color('bg_secondary'),
                             fg=self.theme_manager.get_color('text'),
                             bd=0, highlightthickness=0)
            role_text.pack(fill="x", padx=15, pady=(0, 10))
            
            for point in role_points:
                role_text.insert("end", "• " + point + "\n")
            role_text.configure(state="disabled")
            ...
            messagebox.showerror("Info Error", f"Failed to show Project 89 info: {str(e)}")
            print(f"Project 89 info error: {e}")
    ...
    def create_meditation_player(self, meditation):
        ...
        try:
            ...
            if meditation.get('type') == 'guided' and 'content' in meditation:
                ...
                # Create a styled frame for the content with quantum styling
                content_container = ttk.Frame(content_frame, style="Dark.TFrame")
                content_container.pack(fill='both', expand=True, padx=2, pady=2)
                
                content_text = tk.Text(
                    content_container,
                    height=12,  # Increased height for better content viewing
                    bg=self.theme_manager.get_color('bg_secondary'),
                    fg=self.theme_manager.get_color('text'),
                    font=self.fonts['normal'],
                    wrap='word',
                ...
                scrollbar = ttk.Scrollbar(
                    ...
                    style="Quantum.Vertical.TScrollbar"
                )
                scrollbar.pack(side='right', fill='y')
                ...
                # Auto-scroll to the top to ensure content starts from the beginning
                content_text.yview_moveto(0.0)
    ...
    def launch_immersive_mode(self):
        ...
        try:
            ...
            if self.current_meditation['type'] == 'guided':
                ...
                text_frame.place(relx=0.5, rely=0.8, anchor='center', width=800)
                
                # Create a header for the content
                content_header = tk.Label(
                    text_frame,
                ...
                content_header.pack(pady=(0, 10))
                
                # Create scrollable text area for better content viewing
                ...
                # Add text label with meditation content
                text_label = tk.Label(
                    scrollable_content,
                    text=self.current_meditation['content'],
                    wraplength=780,
                    justify='left',
                    bg=self.theme_manager.get_color('bg'),
                    fg=self.theme_manager.get_color('text'),
                ...
                # Function to cycle focus through paragraphs (highlight the current part being read)
                ...
                def cycle_text():
                    ...
                    # Create a highlighted version of the text with the current paragraph in a different color
                    full_text = self.current_meditation['content']
                    highlighted_text = ""
                    start_idx = 0
                    
                    for i, para in enumerate(paragraphs):
                        if i == current_paragraph[0]:
                            # Apply highlight styling
                            highlighted_part = f"{para}"
                            text_label.config(text=highlighted_part)
                            break
            ...
            def update_immersive_timer():
                if not hasattr(self, 'timer_running') or not self.timer_running:
                    # Timer not running, show full duration
                    remaining = self.meditation_duration
                else:
                    # Timer is running, use remaining_time
