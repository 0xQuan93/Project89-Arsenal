...
class EnchantedMindMirror:
    ...
    def launch_reality_glitcher(self):
        ...
        try:
            # Find Reality Glitcher path
            rg_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "RealityGlitcher")
            
            if os.path.exists(rg_path):
                # Check for launch script
                launch_script = os.path.join(os.path.dirname(rg_path), "launch_reality_glitcher.bat")
                
                if os.path.exists(launch_script):
                    # Use subprocess to run the launch script
                    subprocess.Popen(launch_script, shell=True)
                    self.status_text.config(text="Launched Reality Glitcher")
                    return True
                else:
                    # Try direct Python launch
                    rg_main = os.path.join(rg_path, "main.py")
                    if os.path.exists(rg_main):
                        subprocess.Popen([sys.executable, rg_main], shell=True)
                        self.status_text.config(text="Launched Reality Glitcher")
                        return True
            ...
            messagebox.showinfo(
                ...
                "The Reality Glitcher application could not be found. Your data has been exported and will be available when you run Reality Glitcher manually."
            )
            return False
            
        except Exception as e:
            print(f"Error launching Reality Glitcher: {e}")
            messagebox.showerror("Launch Error", f"Failed to launch Reality Glitcher: {e}")
            return False
    ...
    def update_meditation_stats(self, completed=False):
        ...
        try:
            ...
            # Show completion message if meditation is completed
            if completed:
                messagebox.showinfo("Meditation Complete", 
                                   f"You have completed '{self.current_meditation['title']}' meditation.")
